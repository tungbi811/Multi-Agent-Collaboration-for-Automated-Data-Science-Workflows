...
from multi_agent.crew import CrispDMCrew 
...
class CrispDMFlow(Flow[CrispDMState]):
    ...
    def get_user_input(self):
        """Đọc input từ CLI flags thay vì input() để tránh bị nuốt stdin bởi Flow UI."""
        parser = argparse.ArgumentParser(description="CRISP-DM Flow (non-interactive)")
        parser.add_argument("--topic", required=True, help="Bind vào {topic} trong agents.yaml")
        parser.add_argument("--user_request", required=True, help="Yêu cầu/bài toán kinh doanh")
        parser.add_argument("--dataset", required=True, help="Đường dẫn CSV dataset")
